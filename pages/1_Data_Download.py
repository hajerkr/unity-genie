...
def download_session_data(session,project_path):
...
def download_derivatives(project_id, segtool, input_source, keywords, timestampFilter,fw):
    ...
    # Preallocate lists
    st.session_state.df = pd.DataFrame() #for recon-all
    st.session_state.df2 = pd.DataFrame() #for minimorph
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_session = {
            executor.submit(download_session_data, session, project_path): session 
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_session)):
            ...
            status_text.text(f"Completed {i + 1}/{len(sessions)}: {result}")
    ...
    try:
        ...
        if st.session_state.df.empty and st.session_state.df2.empty:
        ...
        if not st.session_state.df.empty and not st.session_state.df2.empty:
            ...
            st.session_state.df = pd.merge(st.session_state.df, st.session_state.df2, how='outer', on=['subject', 'session', 'acquisition', 'session_qc', 'age', 'age_source','sex','input_gear_v'])
...
def assemble_csv(derivatives, out_csv="derivatives_summary.csv"):
    ...
    st.session_state.df.to_csv(outdir)
    st.session_state.df.to_csv(out_csv, index=False)
...
if st.sidebar.button("Fetch derivatives"):
    ...
    if not all_derivatives:
        ...
        st.session_state.df = assemble_csv(all_derivatives)

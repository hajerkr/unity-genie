...
def process_subject_row(row, segmentation_tool, segmentation_suffix, download_dir, fw, api_key, viz_mode):
    ...
    try:
        sub_label, ses_label = row[st.session_state.subject], row[st.session_state.session]
        project_label        = row[st.session_state.project].strip()
        asys_id = None
...
def load_ratings(ratings_file, metrics, download=False):
    if os.path.exists(ratings_file):
        ...
        if download:
            st.dataframe(df)
        return df
    return pd.DataFrame(columns=["user", "timestamp", st.session_state.project, st.session_state.subject, st.session_state.session] + metrics)
        ...
        if csv_files:
            ...
            for csv_file in csv_files:
                ...
                try:
                    ...
                    if not df.empty and {st.session_state.subject, st.session_state.session, "user"}.issubset(df.columns):
                        st.session_state.asys = latest
                except Exception as e:
...
if not st.session_state.authenticated: #not API_KEY and 
...
with st.sidebar:
    ...
    project_subject_session = st.text_input("Project / Subject / Session (optional)", value="project,subject,session")
    if username:
        st.session_state.username = username.replace(" ", "_")
        st.success(f"Hello, {username}!")
    if project_subject_session:
        st.session_state.project = project_subject_session.split(",")[0].strip()
        st.session_state.subject = project_subject_session.split(",")[1].strip() if len(project_subject_session.split(",")) > 1 else ""
        st.session_state.session = project_subject_session.split(",")[2].strip() if len(project_subject_session.split(",")) > 2 else ""
    ...
    if not st.session_state.data_prepared:
        ...
        if len(project_labels) == 1:
            ...
            if reviewed:
                ...
                rated_pairs = prev_df[[st.session_state.subject, st.session_state.session]].apply(tuple, axis=1).tolist()
                st.session_state.df_outliers["_ss"] = (
                    st.session_state.df_outliers[[st.session_state.subject, st.session_state.session]].apply(tuple, axis=1)
                )
